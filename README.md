...
| `referrer_id` | — | — | **Herkunft-Filter (nur `/articles`).** Nur Artikel, die die angegebene(n) Herkunft/-künfte führen. Mehrere per Komma, z. B. `?referrer_id=11.04,1,104`. Match, wenn **irgendeine** Variante die Herkunft (`variations[].markets[].referrer_id`) trägt. Zweiphasig: **(1)** leichter Katalog-Scan (nur `variationMarkets`) → Treffer-IDs; **(2)** nur die Treffer werden voll geladen/serialisiert. Liefert in **einem** Aufruf **alle** Treffer — **kein** Client-Paginieren nötig. `total_count` = `returned_count` = Treffer-Anzahl, `has_next_page: false`, `meta.filter_applied: "server_side_full_scan"`. Scan-Kappe `MAX_SCAN_PAGES` → ggf. `meta.filter_truncated: true`. `page`/`per_page` werden im Filter-Modus ignoriert. Hinweis: Plenty ignoriert den serverseitigen `referrerId`-Suchparameter, daher der Scan. |
